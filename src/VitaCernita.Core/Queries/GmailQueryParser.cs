...
{
    public static readonly string[] SupportedFieldNames =
    [
        ...
        "has_user_labels", "user_labels",
        "has_no_user_labels", "no_user_labels",
        "is_unread", "unread",
        "is_read", "read",
    ...
    public static IQueryCondition ParseQuery(LuaTable table, string? customDateFormat = null)
    {
        ...
        if (table.TryGetValue("type", out var typeVal) && typeVal.Type == LuaValueType.String)
        {
            ...
            if (t is "query" or "query_builder" or "builder" or "filter_builder")
            {
                ...
                {
                    return ParseConditionsList(condsTable, isOr: false, customDateFormat);
                }
                if (table.TryGetValue("definition", out var defVal))
                {
                    if (defVal.TryRead<LuaTable>(out var defTable))
                    {
                        return ParseCondition(defTable, customDateFormat);
                    }
                    if (defVal.Type == LuaValueType.String)
                    {
                        return new ExactMatchCondition(defVal.Read<string>());
                    }
                }
            }
        }

        // 2. Direct query / match property
        if (table.TryGetValue("query", out var qVal))
        {
            if (qVal.TryRead<LuaTable>(out var qTable))
            {
                return ParseCondition(qTable, customDateFormat);
            }
            if (qVal.Type == LuaValueType.String)
            {
                return new ExactMatchCondition(qVal.Read<string>());
            }
        }

        if (table.TryGetValue("match", out var mVal) && mVal.TryRead<LuaTable>(out var mTable))
        {
            return ParseCondition(mTable, customDateFormat);
        }

        return ParseCondition(table, customDateFormat);
    }

    public static IQueryCondition ParseCondition(LuaTable table, string? customDateFormat = null)
    {
        // 1. Fluent Builder pattern: { type = "builder"|"query_builder"|"filter_builder", conditions = { ... } }
        if (table.TryGetValue("type", out var typeVal) && typeVal.Type == LuaValueType.String &&
            (typeVal.Read<string>() == "builder" || typeVal.Read<string>() == "query_builder" || typeVal.Read<string>() == "filter_builder") &&
        ...
        if (table.TryGetValue("op", out var opVal) && opVal.Type == LuaValueType.String)
        {
            ...
            if (isNot)
            {
                ...
                {
                    throw new QueryValidationException("A 'not' condition cannot be empty; it must negate a valid expression.");
                }
        ...
        // 10. Keyed operators: ["or"], ["any_of"], ["any"]
        ...
        // 11. Keyed operators: ["and"], ["all_of"], ["all"]
        ...
        // 12. Direct fields or string matches on the table
        ...
        // 13. Array of conditions: { condition1, condition2 }
        if (table.ArrayLength > 0)
        {
            return ParseConditionsList(table, isOr: false, customDateFormat);
        }

        throw new LuaConfigException("Unable to parse Gmail query condition from Lua table: no recognized fields or operators found.");
    }

    private static IQueryCondition ParseOperatorBlock(LuaTable blockTable, bool isOr, string? customDateFormat)
    {
    ...
    private static IQueryCondition ParseConditionsList(LuaTable listTable, bool isOr, string? customDateFormat)
    {
        var conditions = new List<IQueryCondition>();
    ...
    public static List<IQueryCondition> ExtractDirectConditions(LuaTable table, string? customDateFormat)
    {
        var conditions = new List<IQueryCondition>();
    ...
    public static IQueryCondition CreateConditionForField(string field, string value, string? customDateFormat)
    {
        ...
        {
            DateTime dt = QueryValidator.ValidateAndParseDate(normField, value, customDateFormat);
        ...
        if (normField is "older_than" or "newer_than" or "older-than" or "newer-than")
        {
            ...
            string dur = QueryValidator.ValidateAndNormalizeDuration(op, value);
    ...
    public static bool HasAnyConditionFields(LuaTable table)
    {
        if (table.TryGetValue("type", out var typeVal))
        {
            ...
            if (t is "action" or "action_builder" or "action_item")
                return false;
            if (t is "query" or "query_builder" or "operator" or "exact" or "field" or "builder" or "has" or "is" or "in" or "category" or "size" or "filter_builder")
                return true;
        }
    ...
    private static bool IsIsKey(string key)
    {
        ...
        norm = norm.Replace('_', '-');
        return QueryValidator.CanonicalIsTargets.Contains(norm);
    }
    ...
    private static string ExtractIsTargetFromKey(string key)
    {
        ...
        norm = norm.Replace('_', '-');
        return QueryValidator.ValidateAndNormalizeIsTarget("is", norm);
    }
    ...
    private static bool IsInKey(string key)
    {
        ...
        return QueryValidator.CanonicalInTargets.Contains(norm);
    }
    ...
    private static string ExtractInTargetFromKey(string key)
    {
        ...
        norm = norm.Replace('_', '-');
        return QueryValidator.ValidateAndNormalizeInTarget("in", norm);
    }
    ...
    private static bool IsCategoryKey(string key)
    {
        ...
        return QueryValidator.CanonicalCategoryTargets.Contains(norm);
    }
    ...
    private static string ExtractCategoryTargetFromKey(string key)
    {
        ...
        norm = norm.Replace('_', '-');
        return QueryValidator.ValidateAndNormalizeCategoryTarget("category", norm);
    }
    ...
    private static bool IsHasKey(string key)
    {
        ...
        return QueryValidator.CanonicalHasTargets.Contains(norm);
    }
    ...
    private static string ExtractHasTargetFromKey(string key)
    {
        ...
        norm = norm.Replace('_', '-');
        if (norm.EndsWith("guillemets")) norm = norm[..^1];
        return QueryValidator.ValidateAndNormalizeHasTarget("has", norm);
    }
